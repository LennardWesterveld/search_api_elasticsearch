...
/**
 * @file
 * Hooks provided by the Search API Elasticsearch search module.
 */
 ...
 */
function hook_search_api_elasticsearch_query_alter(\Solarium\Core\Query\QueryInterface $solarium_query, \Drupal\search_api\Query\QueryInterface $query) {
...
/**
 * Change the way the index's field names are mapped to Elasticsearch field names.
 *
 * @param \Drupal\search_api\IndexInterface $index
 *   The index whose field mappings are altered.
 * @param array $fields
 *   An associative array containing the index field names mapped to their Elasticsearch
 *   counterparts. The special fields 'search_api_id' and 'search_api_relevance'
 *   are also included.
 */
function hook_search_api_elasticsearch_field_mapping_alter(\Drupal\search_api\IndexInterface $index, array &$fields) {
...
/**
 * Alter Elasticsearch documents before they are sent to Elasticsearch for indexing.
 *
 ...
 */
function hook_search_api_elasticsearch_documents_alter(&$documents, \Drupal\search_api\IndexInterface $index, array $items) {
...
/**
 * Lets modules alter the search results returned from a Elasticsearch search.
 *
 ...
 */
function hook_search_api_elasticsearch_search_results_alter(\Drupal\search_api\Query\ResultSetInterface $result_set, \Drupal\search_api\Query\QueryInterface $query, \Solarium\QueryType\Select\Result\Result $result) {
...
/**
 * Provide Elasticsearch dynamic fields as Search API data types.
 *
 ...
 *   - prefix: The Elasticsearch field name prefix to use for this type. Should match
 *     two existing dynamic fields definitions with names "{PREFIX}s_*" and
 *     "{PREFIX}m_*".
 *
 * @see hook_search_api_data_type_info()
 */
function search_api_elasticsearch_hook_search_api_data_type_info() {
  return array(
