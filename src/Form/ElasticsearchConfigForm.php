...
namespace Drupal\search_api_elasticsearch\Form;
...
use Drupal\search_api_elasticsearch\Plugin\search_api\backend\SearchApiElasticsearchBackend;
 ...
 */
class ElasticsearchConfigForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state, ServerInterface $search_api_server = NULL) {
    ...
    try {
      ...
      $files_list = SearchApiElasticsearchUtility::getServerFiles($search_api_server);
      ...
      foreach ($files_list as $file_name => $file_info) {
        ...
        if ($file_info['size'] > 0) {
          ...
          $file_data = $backend->getElasticsearchConnector()->getFile($file_name);
      ...
      watchdog_exception('search_api_elasticsearch', $e, '%type while retrieving config files of Elasticsearch server @server: @message in %function (line %line of %file).', array('@server' => $search_api_server->label()));
  ...
  /**
   * Checks access for the Elasticsearch config form.
   *
  ...
  public function access(ServerInterface $search_api_server) {
    return AccessResult::allowedIf($search_api_server->hasValidBackend() && $search_api_server->getBackend() instanceof SearchApiElasticsearchBackend)->cacheUntilEntityChanges($search_api_server);
  }
