...
use Drupal\search_api_elasticsearch\SearchApiElasticsearchException;
use Drupal\search_api_elasticsearch\ElasticsearchBackendInterface;
use Drupal\search_api_elasticsearch\ElasticsearchConnector\ElasticsearchConnectorPluginManager;
use Drupal\search_api_elasticsearch\Utility\Utility as SearchApiElasticsearchUtility;
...
/**
 * The minimum required Elasticsearch schema version.
 */
...
/**
 * Apache Elasticsearch backend for search api.
 *
 * @SearchApiBackend(
 *   id = "search_api_elasticsearch",
 *   label = @Translation("Elasticsearch"),
 *   description = @Translation("Index items using an Apache Elasticsearch search server.")
 * )
 */
class SearchApiElasticsearchBackend extends BackendPluginBase implements ElasticsearchBackendInterface, PluginFormInterface {
  ...
  /**
   * Metadata describing fields on the Elasticsearch/Lucene index.
   *
   * @var string[][]
   */
  ...
  /**
   * A config object for 'search_api_elasticsearch.settings'.
   *
   ...
   *
   * @var \Drupal\search_api_elasticsearch\ElasticsearchConnector\ElasticsearchConnectorPluginManager
   */
  protected $elasticsearchConnectorPluginManager;

  /**
   * @var \Drupal\search_api_elasticsearch\ElasticsearchConnectorInterface
   */
  protected $elasticsearchConnector;
   ...
   * {@inheritdoc}
   */
  public function __construct(array $configuration, $plugin_id, array $plugin_definition, ModuleHandlerInterface $module_handler, Config $search_api_elasticsearch_settings, LanguageManagerInterface $language_manager, ElasticsearchConnectorPluginManager $elasticsearch_connector_plugin_manager, FieldsHelperInterface $fields_helper, DataTypeHelperInterface $dataTypeHelper) {
    parent::__construct($configuration, $plugin_id, $plugin_definition);

    $this->moduleHandler = $module_handler;
    $this->searchApiElasticsearchSettings = $search_api_elasticsearch_settings;
    $this->languageManager = $language_manager;
    $this->elasticsearchConnectorPluginManager = $elasticsearch_connector_plugin_manager;
  ...
  public static function create(ContainerInterface $container, array $configuration, $plugin_id, $plugin_definition) {
    return new static(
      ...
      $container->get('config.factory')->get('search_api_elasticsearch.settings'),
      $container->get('language_manager'),
      $container->get('plugin.manager.search_api_elasticsearch.connector'),
  ...
  public function buildConfigurationForm(array $form, FormStateInterface $form_state) {
    if (!$this->server->isNew()) {
      ...
      $form['server_description'] = array(
        '#type' => 'item',
        '#title' => $this->t('Elasticsearch server URI'),
        '#description' => $this->getElasticsearchConnector()->getServerLink(),
      );
    }

    $elasticsearch_connector_options = $this->getElasticsearchConnectorOptions();
    $form['connector'] = array(
      ...
      '#options' => $elasticsearch_connector_options,
      '#default_value' => $this->configuration['connector'],
      '#required' => TRUE,
      '#ajax' => array(
        'callback' => [get_class($this), 'buildAjaxElasticsearchConnectorConfigForm'],
        'wrapper' => 'search-api-elasticsearch-connector-config-form',
        'method' => 'replace',
        'effect' => 'fade',
      ),
    );
    ...
    $form['advanced']['retrieve_data'] = array(
      '#type' => 'checkbox',
      '#title' => $this->t('Retrieve result data from Elasticsearch'),
      '#description' => $this->t('When checked, result data will be retrieved directly from the Elasticsearch server. This might make item loads unnecessary. Only indexed fields can be retrieved. Note also that the returned field data might not always be correct, due to preprocessing and caching issues.'),
      '#default_value' => $this->configuration['retrieve_data'],
    );
    $form['advanced']['highlight_data'] = array(
      '#type' => 'checkbox',
      '#title' => $this->t('Highlight retrieved data'),
      '#description' => $this->t('When retrieving result data from the Elasticsearch server, try to highlight the search terms in the returned fulltext fields.'),
      '#default_value' => $this->configuration['highlight_data'],
    );
    $form['advanced']['excerpt'] = array(
      '#type' => 'checkbox',
      '#title' => $this->t('Return an excerpt for all results'),
      '#description' => $this->t("If search keywords are given, use Elasticsearch's capabilities to create a highlighted search excerpt for each result. Whether the excerpts will actually be displayed depends on the settings of the search, though."),
   ...
   *   labels.
   */
  protected function getElasticsearchConnectorOptions() {
    $options = [];
    foreach ($this->elasticsearchConnectorPluginManager->getDefinitions() as $plugin_id => $plugin_definition) {
   ...
   *
   * @param \Drupal\search_api_elasticsearch\ElasticsearchConnectorInterface $connector
  ...
  public function buildConnectorConfigForm(array &$form, FormStateInterface $form_state) {
    ...
    if ($connector_id) {
      $connector = $this->elasticsearchConnectorPluginManager->createInstance($connector_id, $this->configuration['connector_config']);
      if ($connector instanceof PluginFormInterface) {
        $form_state->set('connector', $connector_id);
        if ($form_state->isRebuilding()) {
          drupal_set_message($this->t('Please configure the selected Elasticsearch connector.'), 'warning');
        }
        ...
        $form['connector_config']['#title'] = $this->t('Configure %plugin Elasticsearch connector', array('%plugin' => $connector->label()));
    ...
    $form['connector_config']['#attributes'] = [
      'id' => 'search-api-elasticsearch-connector-config-form',
    ];
  ...
  /**
   * Handles switching the selected Elasticsearch connector plugin.
   */
  public static function buildAjaxElasticsearchConnectorConfigForm(array $form, FormStateInterface $form_state) {
  ...
  public function validateConfigurationForm(array &$form, FormStateInterface $form_state) {
    ...
    if ($form_state->getValue('connector') != $form_state->get('connector')) {
      $new_connector = $this->elasticsearchConnectorPluginManager->createInstance($form_state->getValue('connector'));
    ...
    // Check before loading the backend plugin so we don't throw an exception.
      ...
      $connector = $this->getElasticsearchConnector();
      if ($connector instanceof PluginFormInterface) {
        $connector_form_state = SubformState::createForSubform($form['connector_config'], $form, $form_state);
  ...
  public function submitConfigurationForm(array &$form, FormStateInterface $form_state) {
    ...
    // Highlighting retrieved data and getting an excerpt only makes sense when
    // we retrieve data from the Elasticsearch backend.
  ...
  public function getElasticsearchConnector() {
    if (!$this->elasticsearchConnector) {
      if (!($this->elasticsearchConnector = $this->elasticsearchConnectorPluginManager->createInstance($this->configuration['connector'], $this->configuration['connector_config']))) {
        throw new SearchApiException("The Elasticsearch Connector with ID '$this->configuration['connector']' could not be retrieved.");
      }
    }
    return $this->elasticsearchConnector;
  }
  ...
  public function isAvailable() {
    $conn = $this->getElasticsearchConnector();
  ...
  public function supportsDataType($type) {
    return in_array($type, [
      'location',
      'rpt',
      'elasticsearch_string_ngram',
      'elasticsearch_string_storage',
      'elasticsearch_text_ngram',
      'elasticsearch_text_phonetic',
      'elasticsearch_text_unstemmed',
      'elasticsearch_text_wstoken',
    ]);
  }
  ...
  public function viewSettings() {
    ...
    $info[] = [
      'label' => $this->t('Elasticsearch connector plugin'),
    ...
    $info[] = [
      'label' => $this->t('Elasticsearch core URI'),
    ...
    if ($this->server->status()) {
      ...
      if ($ping_server) {
        ...
        $msg = $this->t('The Elasticsearch server could not be reached or is protected by your service provider.');
      }
      $info[] = [
      ...
      if ($ping) {
        $msg = $this->t('The Elasticsearch core could be accessed (latency: @millisecs ms).', ['@millisecs' => $ping * 1000]);
      }
      else {
        $msg = $this->t('The Elasticsearch core could not be accessed. Further data is therefore unavailable.');
      }
      $info[] = [
      ...
      $version = $connector->getElasticsearchVersion();
      $info[] = [
        'label' => $this->t('Configured Elasticsearch Version'),
        'info' => $version,
      ...
      if ($ping_server || $ping) {
        $info[] = [
          'label' => $this->t('Detected Elasticsearch Version'),
          'info' => $connector->getElasticsearchVersion(TRUE),
          'status' => 'ok',
        ];

        try {
          // If Elasticsearch can be reached, provide more information. This isn't done
          ...
          if (isset($data['index']['numDocs'])) {
            ...
            if (empty($this->configuration['skip_schema_check'])) {
              ...
              elseif (!preg_match('/drupal-[' . SEARCH_API_SOLR_MIN_SCHEMA_VERSION . '-9]\./', $stats_summary['@schema_version'])) {
                $variables['@url'] = Url::fromUri('internal:/' . drupal_get_path('module', 'search_api_elasticsearch') . '/INSTALL.txt')
                  ->toString();
                $message = $this->t('You are using an incompatible schema.xml configuration file. Please follow the instructions in the <a href="@url">INSTALL.txt</a> file for setting up Elasticsearch.', $variables);
            ...
            if (!empty($stats_summary['@core_name'])) {
              $info[] = [
                'label' => $this->t('Elasticsearch Core Name'),
          ...
          $info[] = [
            ...
            'info' => $this->t('An error occurred while trying to retrieve additional information from the Elasticsearch server: %msg', ['%msg' => $e->getMessage()]),
            'status' => 'error',
          ];
        }
      }
  ...
  protected function indexFieldsUpdated(IndexInterface $index) {
    ...
    $old_field_names = $this->getElasticsearchFieldNames($original, TRUE);
    $new_field_names = $this->getElasticsearchFieldNames($index, TRUE);
  ...
  public function indexItems(IndexInterface $index, array $items) {
    ...
    try {
      ...
      watchdog_exception('search_api_elasticsearch', $e, "%type while indexing: @message in %function (line %line of %file).");
  ...
  public function getDocuments(IndexInterface $index, array $items, UpdateQuery $update_query = NULL) {
    ...
    $field_names = $this->getElasticsearchFieldNames($index);
    ...
    foreach ($items as $id => $item) {
      ...
      $doc->setField('hash', SearchApiElasticsearchUtility::getSiteHash());
      ...
      foreach ($item_fields as $name => $field) {
        ...
        if (!isset($field_names[$name])) {
          ...
          \Drupal::logger('search_api_elasticsearch')->warning('Error while indexing: Unknown field %field on the item with ID @id.', $vars);
          $doc = NULL;
          break;
        }
        ...
        if (!array_key_exists($name, $special_fields) && version_compare($schema_version, '4.4', '>=')) {
          ...
          if ($first_value) {
            // Truncate the string to avoid Elasticsearch string field limitation.
    ...
    // Let other modules alter documents before sending them to elasticsearch.
    $this->moduleHandler->alter('search_api_elasticsearch_documents', $documents, $index, $items);
    $this->alterElasticsearchDocuments($documents, $index, $items);
  ...
  public function deleteItems(IndexInterface $index, array $ids) {
    try {
      $index_id = $this->getIndexId($index->id());
      $elasticsearch_ids = array();
      foreach ($ids as $id) {
        $elasticsearch_ids[] = $this->createId($index_id, $id);
      }
      $connector = $this->getElasticsearchConnector();
      $update_query = $connector->getUpdateQuery();
      $update_query->addDeleteByIds($elasticsearch_ids);
      $connector->update($update_query);
    }
    catch (ExceptionInterface $e) {
      throw new SearchApiElasticsearchException($e->getMessage(), $e->getCode(), $e);
    }
  }
  ...
  public function deleteAllIndexItems(IndexInterface $index, $datasource_id = NULL) {
    ...
    $query .= ' +hash:' . $query_helper->escapePhrase(SearchApiElasticsearchUtility::getSiteHash());
    if ($datasource_id) {
      $query .= ' +' . $this->getElasticsearchFieldNames($index)['search_api_datasource'] . ':' . $query_helper->escapePhrase($datasource_id);
    }
    $update_query = $connector->getUpdateQuery();
   ...
   * {@inheritdoc}
   *
   * Options on $query prefixed by 'elasticsearch_param_' will be passed natively to Elasticsearch
   * as query parameter without the prefix. For example you can set the "Minimum
   * Should Match" parameter 'mm' to '75%' like this:
   * @code
   *   $query->setOption('elasticsearch_param_mm', '75%');
   * @endcode
   */
  public function search(QueryInterface $query) {
    ...
    if ($this->configuration['site_hash']) {
      $site_hash = $query_helper->escapePhrase(SearchApiElasticsearchUtility::getSiteHash());
    ...
    if (isset($options['search_api_rpt'])) {
      if (version_compare($connector->getElasticsearchVersion(), 5.1, '>=')) {
        $this->setRpt($solarium_query, $options['search_api_rpt'], $field_names);
      }
      else {
        \Drupal::logger('search_api_elasticsearch')->error('Rpt data type feature is only supported by Elasticsearch version 5.1 or higher.');
      }
    ...
    foreach ($options as $option => $value) {
      if (strpos($option, 'elasticsearch_param_') === 0) {
    ...
    try {
      ...
      $this->moduleHandler->alter('search_api_elasticsearch_query', $solarium_query, $query);
      ...
      if (200 != $response->getStatusCode()) {
        throw new SearchApiElasticsearchException(strip_tags($body), $response->getStatusCode());
      }
      $this->alterElasticsearchResponseBody($body, $query);
      ...
      $this->moduleHandler->alter('search_api_elasticsearch_search_results', $results, $query, $result);
      $this->postQuery($results, $query, $result);
    }
    catch (\Exception $e) {
      throw new SearchApiElasticsearchException($this->t('An error occurred while trying to search with Elasticsearch: @msg.', array('@msg' => $e->getMessage())), $e->getCode(), $e);
    }
  }

  /**
   * Apply workarounds for special Elasticsearch versions before searching.
   *
  ...
  protected function applySearchWorkarounds(Query $solarium_query, QueryInterface $query) {
    ...
    $connector = $this->getElasticsearchConnector();
    $schema_version = $connector->getSchemaVersion();
    $elasticsearch_version = $connector->getElasticsearchVersion();

    // Schema versions before 4.4 set the default query operator to 'AND'. But
    // incompatibilities since Elasticsearch 5.5.0 required a new query builder that
    // bases on 'OR'.
  ...
  /**
   * Creates an ID used as the unique identifier at the Elasticsearch server.
   *
   * This has to consist of both index and item ID. Optionally, the site hash is
   * also included.
   *
   * @see \Drupal\search_api_elasticsearch\Utility\Utility::getSiteHash()
   */
  protected function createId($index_id, $item_id) {
    return SearchApiElasticsearchUtility::getSiteHash() . "-$index_id-$item_id";
  }

  /**
   * {@inheritdoc}
   */
  public function getElasticsearchFieldNames(IndexInterface $index, $reset = FALSE) {
    ...
    if (!isset($this->fieldNames[$index->id()]) || $reset) {
      // This array maps "local property name" => "elasticsearch doc property name".
      $ret = array(
      ...
      foreach ($fields as $key => $field) {
        if (empty($ret[$key])) {
          ...
          // our schema.xml.
          $type = $field->getType();
          $type_info = SearchApiElasticsearchUtility::getDataTypeInfo($type);
          ...
          $name = $pref . '_' . $key;
          $ret[$key] = SearchApiElasticsearchUtility::encodeElasticsearchName($name);

          // Add a distance pseudo field for any location field. These fields
          // don't really exist in the elasticsearch core, but we tell elasticsearch to name the
          // distance calculation results that way. Later we directly pass these
          // as "fields" to Drupal and especially Views.
          if ($type == 'location') {
            $ret[$key . '__distance'] = SearchApiElasticsearchUtility::encodeElasticsearchName($name . '__distance');
          }
        }
      }

      // Let modules adjust the field mappings.
      $this->moduleHandler->alter('search_api_elasticsearch_field_mapping', $index, $ret);
  ...
  /**
   * Applies custom modifications to indexed Elasticsearch documents.
   *
   * This method allows subclasses to easily apply custom changes before the
   * documents are sent to Elasticsearch. The method is empty by default.
   *
   ...
   *
   * @see hook_search_api_elasticsearch_documents_alter()
   */
  protected function alterElasticsearchDocuments(array &$documents, IndexInterface $index, array $items) {
  }

  /**
   * Extract results from a Elasticsearch response.
   *
   * @param \Drupal\search_api\Query\QueryInterface $query
  ...
  protected function extractResults(QueryInterface $query, ResultInterface $result) {
    ...
    $result_set->setExtraData('search_api_elasticsearch_response', $result->getData());
    ...
    foreach ($docs as $doc) {
      ...
      $result_item->setExtraData('search_api_elasticsearch_document', $doc);
      ...
      // Extract properties from the Elasticsearch document, translating from Elasticsearch to
      // Search API property names. This reverses the mapping in
      // SearchApiElasticsearchBackend::getFieldNames().
      foreach ($field_names as $search_api_property => $elasticsearch_property) {
        if (isset($doc_fields[$elasticsearch_property]) && isset($fields[$search_api_property])) {
          $doc_field = is_array($doc_fields[$elasticsearch_property]) ? $doc_fields[$elasticsearch_property] : [$doc_fields[$elasticsearch_property]];
      ...
      if (!empty($backend_config['retrieve_data'])) {
        $elasticsearch_id = $this->createId($index->id(), $result_item->getId());
        $excerpt = $this->getExcerpt($result->getData(), $elasticsearch_id, $result_item, $field_names);
        if ($excerpt) {
    ...
    /*if (module_exists('search_api_spellcheck') && $query->getOption('search_api_spellcheck')) {
       $result_set->setExtraData('search_api_spellcheck', new SearchApiSpellcheckElasticsearch($result));
    }*/
  ...
  /**
   * Serializes a query's conditions as Elasticsearch filter queries.
   *
   * @param \Drupal\search_api\Query\QueryInterface $query
   *   The query to get the conditions from.
   * @param array $elasticsearch_fields
   *   The mapping from Drupal to Elasticsearch field names.
   ...
   */
  protected function getFilterQueries(QueryInterface $query, array $elasticsearch_fields, array $index_fields, array &$options) {
    $condition_group = $query->getConditionGroup();
    $this->addLanguageConditions($condition_group, $query);
    return $this->createFilterQueries($condition_group, $elasticsearch_fields, $index_fields, $options);
  }

  /**
   * Recursively transforms conditions into a flat array of Elasticsearch filter queries.
   *
   * @param \Drupal\search_api\Query\ConditionGroupInterface $condition_group
   ...
   * @throws \Drupal\search_api\SearchApiException
   */
  protected function createFilterQueries(ConditionGroupInterface $condition_group, array $elasticsearch_fields, array $index_fields, array &$options) {
    $fq = [];
    ...
    foreach ($conditions as $condition) {
      if ($condition instanceof ConditionInterface) {
        ...
        $filter_query = $this->createFilterQuery($elasticsearch_fields[$field], $value, $condition->getOperator(), $index_fields[$field], $options);
        if ($filter_query) {
          $fq[] = [
            'query' => $this->createFilterQuery($elasticsearch_fields[$field], $value, $condition->getOperator(), $index_fields[$field], $options),
        ...
        $nested_fqs = $this->createFilterQueries($condition, $elasticsearch_fields, $index_fields, $options);
  ...
  protected function createFilterQuery($field, $value, $operator, FieldInterface $index_field, array &$options) {
    ...
    switch ($operator) {
      ...
      case 'IN':
        ...
        if ($null) {
          // @see https://stackoverflow.com/questions/4238609/how-to-query-elasticsearch-for-empty-fields/28859224#28859224
          return "(*:* -$field:[* TO *])";
        }
  ...
  protected function createLocationFilterQuery(&$spatial) {
    ...
    switch ($spatial['filter_query_conditions']['operator']) {
      ...
      case 'NOT IN':
      default:
        throw new SearchApiElasticsearchException('Unsupported operator for location queries');
    }
  }
  ...
  protected function formatFilterValue($value, $type) {
    ...
    return $this->getElasticsearchConnector()->getQueryHelper()->escapePhrase($value);
  }
  ...
  protected function formatDate($input) {
    ...
    return $this->getElasticsearchConnector()->getQueryHelper()->formatDate($input);
  }
  ...
  protected function setFacets(QueryInterface $query, Query $solarium_query, array $field_names) {
    ...
    foreach ($facets as $info) {
      ...
      if (isset($info['operator']) && strtolower($info['operator']) === 'or') {
        // @see https://cwiki.apache.org/confluence/display/elasticsearch/Faceting#Faceting-LocalParametersforFaceting
  ...
  /**
   * Allow custom changes before converting a SearchAPI query into a Elasticsearch query.
   *
   ...
   * Elasticsearch. Works exactly like hook_search_api_elasticsearch_query_alter().
   *
   ...
   *
   * Works exactly like hook_search_api_elasticsearch_search_results_alter().
   *
   ...
   * @param \Drupal\search_api\Query\QueryInterface $query
   */
  protected function alterElasticsearchResponseBody(&$body, QueryInterface $query) {
  }
  ...
  public function getAutocompleteSuggestions(QueryInterface $query, SearchInterface $search, $incomplete_key, $user_input) {
    ...
    if ($this->configuration['suggest_suffix'] || $this->configuration['suggest_corrections'] || $this->configuration['suggest_words']) {
      $connector = $this->getElasticsearchConnector();
      $elasticsearch_version = $connector->getElasticsearchVersion();
      if (version_compare($elasticsearch_version, '6.5', '=')) {
        \Drupal::logger('search_api_elasticsearch')->error('Elasticsearch 6.5.x contains a bug that breaks the autocomplete feature. Downgrade to 6.4.x or upgrade to 6.6.x.');
        return [];
      }
      ...
      try {
        $fl = [];
        if (version_compare($schema_version, '5.4', '>=')) {
          $elasticsearch_field_names = $this->getElasticsearchFieldNames($query->getIndex());
          ...
          foreach ($fulltext_fields as $fulltext_field) {
            $fl[] = 'terms_' . $elasticsearch_field_names[$fulltext_field];
          }
        }
        else {
          $fl[] = 'spell';
        ...
        watchdog_exception('search_api_elasticsearch', $e);
        return [];
      }
  ...
  public function setConfiguration(array $configuration) {
    ...
    // Update the configuration of the Elasticsearch connector as well by replacing it by
    // a new instance with the latest configuration.
    $this->elasticsearchConnector = NULL;
  }
   ...
   * - If set, the "search_api_elasticsearch.settings.index_prefix" configuration.
   * - If set, the index-specific "search_api_elasticsearch.settings.index_prefix_INDEX"
   *   configuration.
  ...
  protected function getIndexId($machine_name) {
    ...
    $id = $this->searchApiElasticsearchSettings->get('index_prefix_' . $machine_name) . $machine_name;
    // Prepend environment prefix.
    $id = $this->searchApiElasticsearchSettings->get('index_prefix') . $id;
    return $id;
  }
  ...
  public function calculateDependencies() {
    $this->calculatePluginDependencies($this->getElasticsearchConnector());
   ...
   *
   * @param array $data
   *   The data extracted from a Elasticsearch result.
   * @param string $elasticsearch_id
   *   The ID of the result item.
   * @param \Drupal\search_api\Item\ItemInterface $item
   *   The fields of the result item.
   * @param array $field_mapping
   *   Mapping from search_api field names to Elasticsearch field names.
   *
   * @return bool|string
   *   FALSE if no excerpt is returned from Elasticsearch, the excerpt string otherwise.
   */
  protected function getExcerpt($data, $elasticsearch_id, ItemInterface $item, array $field_mapping) {
    if (!isset($data['highlighting'][$elasticsearch_id])) {
      return FALSE;
    }
    $output = '';
    // @todo using the spell field is not the optimal solution.
    // @see https://www.drupal.org/node/2735881
    if (!empty($this->configuration['excerpt']) && !empty($data['highlighting'][$elasticsearch_id]['spell'])) {
      foreach ($data['highlighting'][$elasticsearch_id]['spell'] as $snippet) {
        $snippet = strip_tags($snippet);
        $snippet = preg_replace('/^.*>|<.*$/', '', $snippet);
        $snippet = SearchApiElasticsearchUtility::formatHighlighting($snippet);
    ...
    if (!empty($this->configuration['highlight_data'])) {
      $item_fields = $item->getFields();
      foreach ($field_mapping as $search_api_property => $elasticsearch_property) {
        if (!empty($data['highlighting'][$elasticsearch_id][$elasticsearch_property])) {
          $snippets = [];
          foreach ($data['highlighting'][$elasticsearch_id][$elasticsearch_property] as $value) {
            ...
            $snippets[] = [
              ...
              'replace' => SearchApiElasticsearchUtility::formatHighlighting($value),
            ];
          }
          if ($snippets) {
   ...
   *
   * @return string
   *   A Elasticsearch query string representing the same keys.
   */
  protected function flattenKeys(array $keys) {
    ...
    foreach ($keys as $key_nr => $key) {
      ...
      if (is_array($key)) {
        ...
        $k[] = $this->getElasticsearchConnector()->getQueryHelper()->escapePhrase(trim($key));
      }
    }
    if (!$k) {
      return '';
    ...
    // Formatting the keys into a Elasticsearch query can be a bit complex. Keep in mind
   ...
   *   The elasticsearch fields to be highlighted.
   */
  protected function setHighlighting(Query $solarium_query, QueryInterface $query, $highlighted_fields = []) {
    ...
    if ($highlight || $excerpt) {
      $highlighter = \Drupal::config('search_api_elasticsearch.standard_highlighter');
   ...
   *   The elasticsearch mlt query.
   * @param string $index_id
   *   Elasticsearch specific index ID.
  ...
  protected function getMoreLikeThisQuery(QueryInterface $query, $index_id, $index_fields = [], $fields = []) {
    ...
    foreach ($mlt_options['fields'] as $mlt_field) {
      // Elasticsearch 4 has a bug which results in numeric fields not being supported
      // in MLT queries. Date fields don't seem to be supported at all.
      $version = $this->getElasticsearchConnector()->getElasticsearchVersion();
   ...
   *
   * @throws \Drupal\search_api_elasticsearch\SearchApiElasticsearchException
   */
  protected function setSpatial(Query $solarium_query, array $spatial_options, $field_names = []) {
    if (count($spatial_options) > 1) {
      throw new SearchApiElasticsearchException('Only one spatial search can be handled per query.');
    }

    $spatial = reset($spatial_options);
    $elasticsearch_field = $field_names[$spatial['field']];
    $distance_field = $spatial['field'] . '__distance';
    $elasticsearch_distance_field = $field_names[$distance_field];
    ...
    $solarium_query->addField($elasticsearch_distance_field . ':geodist()');
    ...
    $spatial_query->setField($elasticsearch_field);
    ...
    $solarium_query->createFilterQuery($elasticsearch_field)
      ->setQuery($this->createLocationFilterQuery($spatial));

    // Tell elasticsearch to sort by distance if the field is given by Search API.
    $sorts = $solarium_query->getSorts();
    if (isset($sorts[$elasticsearch_distance_field])) {
      $new_sorts = [];
      foreach ($sorts as $key => $order) {
        if ($key == $elasticsearch_distance_field) {
    ...
    if (!empty($facet_set)) {
      ...
      foreach ($facets as $delta => $facet) {
        ...
        if ($facet_options['field'] != $elasticsearch_distance_field) {
          continue;
        }
  ...
  protected function setRpt(Query $solarium_query, array $rpt_options, $field_names = array()) {
    ...
    $elasticsearch_field = $field_names[$rpt['field']];
    $rpt['geom'] = isset($rpt['geom']) ? $rpt['geom'] : '["-180 -90" TO "180 90"]';

    // Add location filter
    $solarium_query->createFilterQuery($elasticsearch_field)->setQuery($elasticsearch_field . ':' . $rpt['geom']);

    // Add Elasticsearch Query params
    $solarium_query->addParam('facet', 'on');
    $solarium_query->addParam('facet.heatmap', $elasticsearch_field);
  ...
  protected function setSorts(Query $solarium_query, QueryInterface $query, $field_names = []) {
    $new_schema_version = version_compare($this->getElasticsearchConnector()->getSchemaVersion(), '4.4', '>=');
    foreach ($query->getSorts() as $field => $order) {
      ...
      if (strpos($field, 'search_api_') === 0) {
        if ($field == 'search_api_random') {
          // The default Elasticsearch schema provides a virtual field named "random_*"
      ...
      elseif ($new_schema_version) {
        ...
        //   SearchApiElasticsearchBackend::getDocuments(). They should be combined in a
