...
use Drupal\search_api_elasticsearch\ElasticsearchConnectorInterface;
use Solarium\Client;
...
/**
 * Defines a base class for Elasticsearch connector plugins.
 *
 * Plugins extending this class need to define a plugin definition array through
 * annotation. These definition arrays may be altered through
 * hook_search_api_elasticsearch_connector_info_alter(). The definition includes the
 * following keys:
 ...
 * @see \Drupal\search_api_elasticsearch\ElasticsearchConnector\ElasticsearchConnectorPluginManager
 * @see \Drupal\search_api_elasticsearch\ElasticsearchConnectorInterface
 * @see plugin_api
 */
abstract class ElasticsearchConnectorPluginBase extends ConfigurablePluginBase implements ElasticsearchConnectorInterface, PluginFormInterface {
  ...
  /**
   * A connection to the Elasticsearch server.
   *
   * @var \Solarium\Client
   */
  ...
  public function buildConfigurationForm(array $form, FormStateInterface $form_state) {
    ...
    $form['host'] = array(
      ...
      '#description' => $this->t('The host name or IP of your Elasticsearch server, e.g. <code>localhost</code> or <code>www.example.com</code>.'),
    ...
    $form['port'] = array(
      '#type' => 'textfield',
      '#title' => $this->t('Elasticsearch port'),
    ...
    $form['path'] = array(
      '#type' => 'textfield',
      '#title' => $this->t('Elasticsearch path'),
      '#description' => $this->t('The path that identifies the Elasticsearch instance to use on the server.'),
      '#default_value' => isset($this->configuration['path']) ? $this->configuration['path'] : '',
    );

    $form['core'] = array(
      '#type' => 'textfield',
      '#title' => $this->t('Elasticsearch core'),
      '#description' => $this->t('The name that identifies the Elasticsearch core to use on the server.'),
    ...
    $form['timeout'] = array(
      ...
      '#description' => $this->t('The timeout in seconds for search queries sent to the Elasticsearch server.'),
    ...
    $form['index_timeout'] = array(
      ...
      '#description' => $this->t('The timeout in seconds for indexing requests to the Elasticsearch server.'),
    ...
    $form['optimize_timeout'] = array(
      ...
      '#description' => $this->t('The timeout in seconds for background index optimization queries on a Elasticsearch server.'),
    ...
    $form['commit_within'] = array(
      ...
      '#description' => $this->t('The limit in milliseconds within a (soft) commit on Elasticsearch is forced after any updating the index in any way. Setting the value to "0" turns off this dynamic enforcement and lets Elasticsearch behave like configured elasticsearchconf.xml.'),
    ...
    $form['workarounds']['elasticsearch_version'] = array(
      '#type' => 'select',
      '#title' => $this->t('Elasticsearch version override'),
      '#description' => $this->t('Specify the Elasticsearch version manually in case it cannot be retrived automatically. The version can be found in the Elasticsearch admin interface under "Elasticsearch Specification Version" or "elasticsearch-spec"'),
      '#options' => array(
      ...
      '#default_value' => isset($this->configuration['elasticsearch_version']) ? $this->configuration['elasticsearch_version'] : '',
    );
  ...
  public function validateConfigurationForm(array &$form, FormStateInterface $form_state) {
    ...
    if (!$form_state->hasAnyErrors()) {
      ...
      $elasticsearch = new Client(NULL, $this->eventDispatcher);
      $elasticsearch->createEndpoint($values + ['key' => 'core'], TRUE);
      try {
        $this->getServerLink();
      }
  ...
  /**
   * Prepares the connection to the Elasticsearch server.
   */
  protected function connect() {
    if (!$this->elasticsearch) {
      $this->elasticsearch = new Client(NULL, $this->eventDispatcher);
      $this->elasticsearch->createEndpoint($this->configuration + ['key' => 'core'], TRUE);
      $this->attachServerEndpoint();
    }
  }

  /**
   * Attaches an endpoint to the Elasticsearch connection to communicate with the server.
   *
   ...
   * This function is meant to be overwritten as soon as we deal with Elasticsearch
   * service provider specific implementations of ElasticsearchHelper.
   */
  protected function attachServerEndpoint() {
    ...
    $this->elasticsearch->createEndpoint($configuration);
  }

  /**
   * Returns a the Elasticsearch server URI.
   */
  protected function getServerUri() {
    $this->connect();
    $url_path = $this->elasticsearch->getEndpoint('server')->getBaseUri();
   ...
   * {@inheritdoc}
   */
  public function getElasticsearchVersion($force_auto_detect = FALSE) {
    // Allow for overrides by the user.
    if (!$force_auto_detect && !empty($this->configuration['elasticsearch_version'])) {
      // In most cases the already stored elasticsearch_version is just the major version
      // number as integer. In this case we will expand it to the minimum
      // corresponding full version string.
      $min_version = ['0', '0', '0'];
      $version = explode('.', $this->configuration['elasticsearch_version']) + $min_version;
    ...
    catch (SearchApiElasticsearchException $e) {
      try {
    ...
    // Get our elasticsearch version number.
    if (isset($info['lucene']['elasticsearch-spec-version'])) {
      return $info['lucene']['elasticsearch-spec-version'];
    }
   ...
   * {@inheritdoc}
   */
  public function getElasticsearchMajorVersion($version = '') {
    list($major, ,) = explode('.', $version ?: $this->getElasticsearchVersion());
    return $major;
  }
   ...
   * {@inheritdoc}
   */
  public function getElasticsearchBranch($version = '') {
    return $this->getElasticsearchMajorVersion($version) . '.x';
  }
  ...
  public function getLuceneMatchVersion($version = '') {
    list($major, $minor,) = explode('.', $version ?: $this->getElasticsearchVersion());
    return $major . '.' . $minor;
  }
  ...
  /**
   * Gets data from a Elasticsearch endpoint using a given handler.
   *
   * @param $endpoint_name
   * @param $handler
   * @param bool $reset
  ...
  protected function getDataFromHandler($endpoint_name, $handler, $reset = FALSE) {
    ...
    $endpoint = $this->elasticsearch->getEndpoint($endpoint_name);
    $endpoint_uri = $endpoint->getBaseUri();
    $state_key = 'search_api_elasticsearch.endpoint.data';
    $state = \Drupal::state();
    ...
    if (!isset($previous_calls[$endpoint_uri][$handler]) || $reset) {
      ...
      if (!is_array($endpoint_data) || !isset($endpoint_data[$endpoint_uri][$handler]) || $reset) {
        ...
        $query = $this->elasticsearch->createPing(array('handler' => $handler));
  ...
  /**
   * Pings the Elasticsearch server to tell whether it can be accessed.
   *
   * @param string $endpoint_name
   *   The endpoint to be pinged on the Elasticsearch server.
   *
   * @return mixed
  ...
  protected function doPing($options = [], $endpoint_name = 'core') {
    ...
    $query = $this->elasticsearch->createPing($options);

    try {
      $start = microtime(TRUE);
      $result = $this->elasticsearch->execute($query, $endpoint_name);
  ...
  public function getStatsSummary() {
    ...
    $query = $this->elasticsearch->createPing();
    $query->setResponseWriter(Query::WT_PHPS);
    $query->setHandler('admin/mbeans?stats=true');
    $stats = $this->execute($query)->getData();
    if (!empty($stats)) {
      $update_handler_stats = $stats['elasticsearch-mbeans']['UPDATEHANDLER']['updateHandler']['stats'];
      ...
      $summary['@core_name'] = $stats['elasticsearch-mbeans']['CORE']['core']['stats']['coreName'];
      if (version_compare($this->getElasticsearchVersion(), '6.4', '>=')) {
        // @see https://issues.apache.org/jira/browse/SOLR-3990
        $summary['@index_size'] = $stats['elasticsearch-mbeans']['CORE']['core']['stats']['size'];
      }
      else {
        $summary['@index_size'] = $stats['elasticsearch-mbeans']['QUERYHANDLER']['/replication']['stats']['indexSize'];
      }
    }
    return $summary;
  }
  ...
  /**
   * Sends a REST request to the Elasticsearch server endpoint and returns the result.
   *
  ...
  protected function restRequest($endpoint_key, $path, $method = Request::METHOD_GET, $command_json = '') {
    ...
    $endpoint = $this->elasticsearch->getEndpoint($endpoint_key);
    ...
    // \Drupal::logger('search_api_elasticsearch')->info(print_r($output, true));.
    if (!empty($output['errors'])) {
      throw new SearchApiElasticsearchException('Error trying to send a REST request.' .
  ...
  public function getUpdateQuery() {
    $this->connect();
    return $this->elasticsearch->createUpdate();
  }
  ...
  public function getSelectQuery() {
    $this->connect();
    return $this->elasticsearch->createSelect();
  }
  ...
  public function getMoreLikeThisQuery() {
    $this->connect();
    return $this->elasticsearch->createMoreLikeThis();
  }
  ...
  public function getTermsQuery() {
    $this->connect();
    return $this->elasticsearch->createTerms();
  }
  ...
  public function getExtractQuery() {
    $this->connect();
    return $this->elasticsearch->createExtract();
  }
  ...
  protected function customizeRequest() {
    $this->connect();
    return $this->elasticsearch->getPlugin('customizerequest');
  }
  ...
  public function search(Query $query, Endpoint $endpoint = NULL) {
    ...
    if (!$endpoint) {
      $endpoint = $this->elasticsearch->getEndpoint('core');
    }
    ...
    if ($this->configuration['http_method'] == 'AUTO') {
      $this->elasticsearch->getPlugin('postbigrequest');
    }

    // Use the manual method of creating a Solarium request so we can control
    // the HTTP method.
    $request = $this->elasticsearch->createRequest($query);
  ...
  public function createSearchResult(Query $query, Response $response) {
    return $this->elasticsearch->createResult($query, $response);
  }
  ...
  public function update(UpdateQuery $query, Endpoint $endpoint = NULL) {
    ...
    if ($this->configuration['commit_within']) {
      // Do a commitWithin since that is automatically a softCommit since Elasticsearch 4
      // and a delayed hard commit with Elasticsearch 3.4+.
      // By default we wait 1 second after the request arrived for elasticsearch to parse
      // the commit. This allows us to return to Drupal and let Elasticsearch handle what
      // it needs to handle.
      // @see http://wiki.apache.org/elasticsearch/NearRealtimeSearch
  ...
  public function execute(QueryInterface $query, Endpoint $endpoint = NULL) {
    ...
    try {
      return $this->elasticsearch->execute($query, $endpoint);
    }
    catch (HttpException $e) {
  ...
  public function executeRequest(Request $request, Endpoint $endpoint = NULL) {
    ...
    try {
      return $this->elasticsearch->executeRequest($request, $endpoint);
    }
    catch (HttpException $e) {
      $this->handleHttpException($e, $endpoint);
    }
  ...
  /**
   * Converts a HttpException in an easier to read SearchApiElasticsearchException.
   *
  ...
  protected function handleHttpException(HttpException $e, Endpoint $endpoint) {
    ...
    throw new SearchApiElasticsearchException($this->t('Elasticsearch endpoint @endpoint @description.', ['@endpoint' => $endpoint->getBaseUri(), '@description' => $description]), $response_code, $e);
  }
  ...
  public function optimize(Endpoint $endpoint = NULL) {
    ...
    $update_query = $this->elasticsearch->createUpdate();
  ...
  public function getEndpoint($key = 'core') {
    $this->connect();
    return $this->elasticsearch->getEndpoint($key);
  }
  ...
  public function __sleep() {
    // It's safe to unset the elasticsearch client completely before serialization
    // because connect() will set it up again correctly after deserialization.
    unset($this->elasticsearch);
